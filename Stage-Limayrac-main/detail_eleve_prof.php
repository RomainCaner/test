...
<body>
    ...
    <main>
        <section class="stage-list">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if ($row['idStatut'] >= 3 && $row['idStatut'] != 5): ?>
                                            <br>
                                            <a href="generate_cs.php?idStage=<?php echo htmlspecialchars($row['idStage']); ?>"
                                                class="download-link"
                                                target="_blank">
                                                Convention
                                            </a>
                                        ...
                                        <?php if ($row['idStatut'] >= 4 && $row['idStatut'] != 5): ?>
                                            <br>
                                            <a href="#"
                                                onclick="openGratificationPopup(<?php echo htmlspecialchars($row['idStage']); ?>)"
                                                class="download-link">
                                                Attestation
                                            </a>
                                        <?php endif; ?>
            ...
            <br>
            <form method="post" action="index_prof.php">
...
<!-- Ajouter une pop-up pour la saisie du montant -->
<div id="popup-gratification" class="popup-modal">
    <h3>Saisir le montant de la gratification</h3>
    <form id="form-gratification" onsubmit="handleGratificationSubmission(event)">
        <input type="hidden" name="idStage" id="gratificationIdStageHidden">
        <label for="gratificationAmount">Montant (€) :</label>
        <input type="number" id="gratificationAmount" name="gratification" step="0.01" required>
        <br><br>
        <button type="submit">Télécharger</button>
        <button type="button" onclick="closeGratificationPopup()">Annuler</button>
    </form>
</div>


<!-- Mettre à jour le lien Attestation -->
<script>
    // Ouvrir la pop-up
    function openGratificationPopup(idStage) {
        document.getElementById("gratificationIdStageHidden").value = idStage;
        document.getElementById("modal-overlay").style.display = "block";
        document.getElementById("popup-gratification").style.display = "block";
    }

    // Fermer la pop-up
    function closeGratificationPopup() {
        document.getElementById("modal-overlay").style.display = "none";
        document.getElementById("popup-gratification").style.display = "none";
    }

    // Gérer la soumission du formulaire
    function handleGratificationSubmission(event) {
        event.preventDefault(); // Empêche la soumission par défaut du formulaire

        const idStage = document.getElementById("gratificationIdStageHidden").value;
        const amount = document.getElementById("gratificationAmount").value;
        ...
        // Soumettre les données et télécharger le PDF
        const form = document.createElement("form");
        form.method = "post";
        form.action = "generate_as.php"; // Action pour mise à jour et génération du PDF
        form.target = "_blank"; // Ouvre dans un nouvel onglet

        const inputIdStage = document.createElement("input");
        inputIdStage.type = "hidden";
        inputIdStage.name = "idStage";
        inputIdStage.value = idStage;
        ...
        inputAmount.name = "gratification";
        inputAmount.value = amount;

        const inputAction = document.createElement("input");
        inputAction.type = "hidden";
        inputAction.name = "action";
        inputAction.value = "update_and_download";
        ...
        // Fermer la pop-up avant soumission
        closeGratificationPopup();
        form.submit();
    }
</script>